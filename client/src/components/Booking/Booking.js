...
const Booking = () => {
  ...
  <div>
    ...
    <Link style={{textDecoration:'none'}} to={(1*c) ===0 ? '' :'/booking/checkout'} state={{data ,c, details}}>
        <Button disabled={(1*c) ===0 ? true : false} variant='outlined' color='primary'> Book Tickets ({1*c})</Button>
    </Link>
    </div> 
    </div>
  )}

export default Auth(Booking, null)